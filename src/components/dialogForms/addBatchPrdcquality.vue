<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/prdc_quality' }">产品自检</el-breadcrumb-item>
        ...
        if (tabJson && tabJson.length > 0) {
          for (var i = 0; i < tabJson[0].sheet.length; i++) {
            ...
            var _row = {
              企业名称: _sheet[i].企业名称,
              生产编号: _sheet[i].生产编号,
              自检日期: that.$CommonUtil.converExcelDate(_sheet[i].自检日期),
              温度: _sheet[i].温度,
              水分: _sheet[i].水分,
              有机质: _sheet[i].有机质,
              自检人: _sheet[i].自检人,
    ...
    async isRecordExit(serviceid, rettingbatch) {
      ...
      return new Promise(function(resolve, reject) {
        if (that.$CommonUtil.isEmptyString(serviceid)) {
          ...
          that.$axios
            .get(
              'http://119.3.221.188:8808/tfDbOperator-2.0/tfProquality/isExist?rettingbatch=' +
                rettingbatch +
                '&serviceid=' +
                serviceid
            )
