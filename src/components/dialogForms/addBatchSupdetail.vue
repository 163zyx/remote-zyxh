<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/supdetail' }"
        >供应明细表</el-breadcrumb-item
      >
    ...
    <el-card style="height: 50vh">
      ...
      <el-row>
        ...
        >
          ...
          <el-table-column prop="批次单号" label="批次单号"> </el-table-column>
          <el-table-column prop="乡镇" label="乡镇"> </el-table-column>
          ...
          <el-table-column prop="作物" label="作物" width="100">
          </el-table-column>
          <el-table-column prop="施用面积" label="施用面积（亩）" width="120">
            <template slot-scope="scope">
              {{ fun(scope.row.施用面积) }}
            </template>
          </el-table-column>
          <el-table-column prop="施用量" label="施用量（吨）" width="120">
            <template slot-scope="scope">
              {{ fun(scope.row.施用量) }}
            </template>
          </el-table-column>
    ...
    async isRecordExit(serviceid, ordernumber) {
      ...
      return new Promise(function (resolve, reject) {
        if (that.$CommonUtil.isEmptyString(serviceid)) {
          ...
          var _url =
            "http://119.3.221.188:8808/tfDbOperator-2.0/tfSupdetail/isExist?servicesubject=" +
            serviceid +
            "&ordernumber=" +
            ordernumber;
    ...
    async insertOneRecord(inputelment) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var poststr = that.$qs.stringify({
          ...
          town: inputelment.乡镇,
          village: inputelment.村,
          name: inputelment.姓名,
          contactinfo: inputelment.联系方式,
          crop: inputelment.作物,
          area: inputelment.施用面积,
          amount: inputelment.量,
      ...
      for (var i = 0; i < that.tableFromExcel.length; i++) {
        ...
        if (_sid != null) {
          if (_sid != "") {
            var poststr = that.$qs.stringify({
              servicesubject: _sid,
              ordernumber: that.tableFromExcel[i].批次单号,
              town: that.tableFromExcel[i].乡镇,
              village: that.tableFromExcel[i].村,
              name: that.tableFromExcel[i].姓名,
              contactinfo: that.tableFromExcel[i].联系方式,
              crop: that.tableFromExcel[i].作物,
              area: that.tableFromExcel[i].施用面积,
              amount: that.tableFromExcel[i].量,
              landlocation: that.tableFromExcel[i].地块位置,
            });
            // postData.push(_row);
