<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/yl_incomes' }"
        >原料进厂台账</el-breadcrumb-item
      >
    ...
    <el-card style="height: 50vh">
      ...
      <el-row>
        ...
        >
          ...
          <el-table-column prop="服务主体" label="服务主体"> </el-table-column>
          <el-table-column prop="日期" label="日期" :formatter="formatDate">
          </el-table-column>
          <el-table-column prop="原料来源地" label="原料来源地">
          </el-table-column>
          <el-table-column prop="原料类型" label="原料类型"> </el-table-column>
          ...
          <el-table-column prop="联系方式" label="联系方式">
            <template scope="scope">
              <span :style="phoneRight(scope.row.联系方式)">{{
                scope.row.联系方式
              }}</span></template
            >
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async insertOneRecord(inputelment) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var postData = that.$qs.stringify({
          serviceid: _sid,
          materialdate: inputelment.日期,
          materialsource: inputelment.原料来源地,
          materialtype: inputelment.原料类型,
          materialquantity: inputelment.数量,
          materialunitprice: inputelment.单价,
          materialamount: inputelment.金额,
          principal: inputelment.负责人,
    ...
    //   var successCount = 0;
    //   that.tableFromExcel.forEach((element) => {
    //     var _sid = that.getServiceIdByName(element.服务主体);
    //     if (_sid != null) {
    //       if (_sid != "") {
    //         if (that.$CommonUtil.isRealDate(element.日期)) {
    //           var poststr = that.$qs.stringify({
    //             serviceid: _sid,
    //             materialdate: element.日期,
    //             materialsource: element.原料来源地,
    //             materialtype: element.原料类型,
    //             materialquantity: element.数量,
    //             materialunitprice: element.单价,
    //             materialamount: element.金额,
    //             principal: element.负责人,
    //             contactinfo: element.联系方式,
    //             note: element.备注,
    //           });
    //           // postData.push(_row);
