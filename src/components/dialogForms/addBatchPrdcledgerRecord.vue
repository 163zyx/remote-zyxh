textarea<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/prdc_ledger' }">生产记录台账</el-breadcrumb-item>
        ...
        if (tabJson && tabJson.length > 0) {
          ...
          for (var i = 0; i < tabJson[0].sheet.length; i++) {
            ...
            var _row = {
              堆沤批次: _sheet[i].堆沤批次,
              服务主体: _sheet[i].服务主体,
              // 堆沤开始日期: that.converExcelDate(_sheet[i].堆沤开始日期),
              // 堆沤结束日期: that.converExcelDate(_sheet[i].堆沤结束日期),
              堆沤开始日期: that.$CommonUtil.converExcelDate(
                _sheet[i].堆沤开始日期
              ),
              堆沤结束日期: that.$CommonUtil.converExcelDate(
                _sheet[i].堆沤结束日期
              ),
              堆沤场所: _sheet[i].堆沤场所,
              原材料辅料: _sheet[i].原材料辅料,
              产量: _sheet[i].产量,
              生产负责人: _sheet[i].生产负责人,
              联系电话: _sheet[i].联系电话,
              状态: ''
            }
    ...
    async isRecordExit(serviceid, rettingbatch) {
      ...
      return new Promise(function(resolve, reject) {
        if (that.$CommonUtil.isEmptyString(serviceid)) {
          ...
          that.$axios
            .get(
              'http://119.3.221.188:8808/tfDbOperator-2.0/tfProductionrecordledger/isExist?rettingbatch=' +
                rettingbatch +
                '&serviceid=' +
                serviceid
            )
    ...
    async insertOneRecord(inputelment) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        var postData = that.$qs.stringify({
          ...
          rettingstartdate: inputelment.堆沤开始日期,
          rettingenddate: inputelment.堆沤结束日期,
          rettingplace: inputelment.堆沤场所,
          materialweight: inputelment.原材料辅料,
          outputweight: inputelment.产量,
          productionprincipal: inputelment.生产负责人,
          contactinfo: inputelment.联系电话
        })
