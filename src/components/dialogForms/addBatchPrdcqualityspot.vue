<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/prdc_qualityspot' }"
        >产品抽检</el-breadcrumb-item
      >
    ...
    <el-card style="height: 50vh">
      ...
      <el-row>
        ...
        >
          ...
          <el-table-column prop="生产编号" label="生产编号"> </el-table-column>
          <el-table-column
            prop="抽检日期"
            label="抽检日期"
          ...
          <el-table-column prop="温度" label="温度"> </el-table-column>
          <el-table-column prop="水分" label="水分"> </el-table-column>
          <el-table-column prop="有机质" label="有机质"> </el-table-column>
          <el-table-column prop="抽检人" label="抽检人"> </el-table-column>
          <el-table-column prop="抽检单位" label="抽检单位"> </el-table-column>
        ...
        if (tabJson && tabJson.length > 0) {
          for (var i = 0; i < tabJson[0].sheet.length; i++) {
            ...
            var _row = {
              企业名称: _sheet[i].企业名称,
              生产编号: _sheet[i].生产编号,
              抽检日期: that.$CommonUtil.converExcelDate(_sheet[i].抽检日期),
              温度: _sheet[i].温度,
              水分: _sheet[i].水分,
              有机质: _sheet[i].有机质,
              抽检人: _sheet[i].抽检人,
    ...
    async findSameRecord() {
      ...
      that.tableFromExcel.forEach((element) => {
        let result = that.isRecordExit(
          that.getServiceIdByName(element.企业名称),
          element.生产编号
        );
        data.push(result);
      });
    ...
    async isRecordExit(serviceid, rettingbatch) {
      ...
      return new Promise(function (resolve, reject) {
        if (that.$CommonUtil.isEmptyString(serviceid)) {
          ...
          that.$axios
            .get(
              "http://119.3.221.188:8808/tfDbOperator-2.0/tfProqualityspot/isExist?rettingbatch=" +
                rettingbatch +
                "&serviceid=" +
                serviceid
            )
    ...
    async insertOneRecord(inputelment) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var postData = that.$qs.stringify({
          serviceid: _sid,
          rettingbatch: inputelment.生产编号,
          monitordate: inputelment.抽检日期,
          temperature: inputelment.温度,
          water: inputelment.水分,
          oganic: inputelment.有机质,
          monitorpeo: inputelment.抽检人,
          monitorunit: inputelment.抽检单位,
          note: inputelment.备注,
        });
