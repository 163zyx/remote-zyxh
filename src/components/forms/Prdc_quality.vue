<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>产品自检</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图 -->
    <el-card style="height: 85vh">
      ...
      >
        ...
        <el-table-column prop="rettingbatch" label="生产编号">
        </el-table-column>
        <el-table-column
          prop="monitordate"
          label="自检日期"
        ...
        <el-table-column prop="oganic" label="有机质"> </el-table-column>

        <el-table-column prop="monitorpeo" label="自检人"> </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="自检日期" prop="monitordate">
              <el-date-picker
                style="width: 100%"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="自检人" prop="monitorpeo">
              <el-input
...
export default {
  ...
  data() {
    return {
      ...
      titleMap: {
        addSensor: "新增自检记录", //根据自己内容定义名字
        eidtSensor: "编辑自检记录", //根据自己内容定义名字
      },
  ...
  methods: {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.dialogStatus == "addSensor") {
            ...
            that.$axios
              .post(
                "http://119.3.221.188:8808/tfDbOperator-2.0/tfProquality/insert",
                postData
              )
              .then((res) => {
                that.$message({
                  message: "成功新增一条产品自检记录！",
                  type: "success",
                });
              ...
              .catch((err) => {
                that.$message.error("新增产品自检记录出错！请认真核对表单信息");
                console.log(err);
              });
            ...
            that.$axios
              .post(
                "http://119.3.221.188:8808/tfDbOperator-2.0/tfProquality/update",
                postData
              )
              .then((res) => {
                that.$message({
              ...
              .catch((err) => {
                that.$message.error("修改产品自检记录出错！请认真核对表单信息");
                console.log(err);
              });
          }
        } else {
    ...
    deleteOnePrcdQuality(row) {
      ...
      that
        ...
        .then(() => {
          ...
          that.$axios
            .get(
              "http://119.3.221.188:8808/tfDbOperator-2.0/tfProquality/deleteById",
              {
                params,
              }
            )
    ...
    exportExcel() {
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, "产品自检表"); // 导出的表格名称，根据需要自己命名
      });
    }, //格式转换，直接复制即可
    ...
    addBatch() {
      var that = this;
      that.$router.replace("../addBatchPrdcquality");
    },
