<template>
  <div class="container">
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{ path: '/prdc_confirm' }"
        >产量确认表</el-breadcrumb-item
      >
      ...
      <el-col :span="16">
        <el-card class="box-card-left">
          ...
          <div id="pieChart" class="pieChart">
            <ve-pie :data="pieData" :extend="piechartExtend"></ve-pie>
          </div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card class="box-card-right">
          ...
          >
            <el-table-column prop="servicesub" label="服务主体">
            </el-table-column>
            <el-table-column prop="convertweight" label="折算吨数">
  ...
  methods: {
    ...
    async serviceSelect() {
      ...
      try {
        ...
        var prdconfirmRes = await this.getPrdConfirmData();
        ...
        for (var i = 0; i < prdconfirmRes.length; i++) {
          var rowObj = {
            servicesub: this.getServiceNameByID(prdconfirmRes[i].serviceid),
            productionconfirmdate: prdconfirmRes[i].productionconfirmdate,
            convertweight: prdconfirmRes[i].convertweight,
          };

          if (this.mainTable) this.mainTable.push(rowObj);
        }

        this.filterTable = this.mergeConvertweight(this.mainTable);
      ...
      for (var i = 0; i < inputTable.length; i++) {
        ...
        if (!map[ai.servicesub]) {
          dest.push({
            ...
            productionconfirmdate: ai.productionconfirmdate,
            convertweight: ai.convertweight,
          });
          ...
          for (var j = 0; j < dest.length; j++) {
            ...
            if (dj.servicesub == ai.servicesub) {
              dj.convertweight = (
                parseFloat(dj.convertweight) + parseFloat(ai.convertweight)
              ).toString();
              break;
            }
          }
    ...
    createPieData() {
      ...
      for (var i = 0; i < this.filterTable.length; i++) {
        var obj = {
          ...
          折算吨数: this.filterTable[i].convertweight,
        };
